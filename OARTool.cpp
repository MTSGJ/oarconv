    ...
    if (!file_exist((char*)outpath.buf)) {
        ...
        if (path!=NULL && extn!=NULL) {
            ...
            // JPEG2000
                ...
                // Repair
                if (jpg.state!=0) {
                    PRINT_MESG("OARTool::ConvertTexture: INFO: failure to read %s. attempt to repair the file.\n", path);
                    ...
                    if (err==JBXL_NORMAL) {
                        ...
                        DEBUG_MODE PRINT_MESG("OARTool::ConvertTexture: ERROR: file repair error (%d).\n", err);
                    }
